...
export const useFileUpload = (
  ...
  const addFiles = useCallback(
    (newFiles: FileList | File[]) => {
      ...
      setState((prev) => {
        // In single file mode, clean up existing files first
        if (!multiple) {
          for (const file of prev.files) {
            if (
              file.preview &&
        ...
        for (const file of newFilesArray) {
          ...
          if (multiple) {
            ...
            // Skip duplicate files silently - use continue, not return!
            if (isDuplicate) {
              continue // <-- FIXED: was 'return' which broke everything
            }
          }
