...
> {
  ...
  modifiers?: Modifiers
  asChild?: boolean
  overlay?: boolean // ✅ NEW: Control built-in overlay
}
...
function Sortable<T>({
  ...
  onDragEnd,
  modifiers,
  overlay = true, // ✅ Default to true for backward compatibility
  children,
  ...props
}: SortableRootProps<T>) {
  ...
  // Find the active child for the overlay (only used if overlay=true)
  ...
  return (
    <SortableInternalContext.Provider value={contextValue}>
      ...
      >
        ...
        {/* ✅ Only render built-in overlay if overlay prop is true */}
        {overlay && mounted &&
          createPortal(
            <DragOverlay
  ...
  return (
    ...
    >
      <Comp
        ...
        {...attributes}
        className={cn(
          isSortableDragging && "z-50 opacity-40", // ✅ Reduced opacity
          disabled && "opacity-50",
          className
        )}
        {...props}
